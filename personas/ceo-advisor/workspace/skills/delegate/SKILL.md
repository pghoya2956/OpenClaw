...
```
/delegate to=<agentId> topic=<주제> prompt="<질문>"
/delegate to=<id1>,<id2> topic=<주제> prompt="<질문>" mode=parallel
```

`topic`은 위임 주제를 식별하는 짧은 키워드입니다. 같은 topic으로 호출하면 전문가가 이전 대화를 기억합니다.

```
/delegate to=product-leader topic=q3-strategy prompt="시장 분석해줘"
/delegate to=product-leader topic=q3-strategy prompt="위 분석 기반으로 우선순위 제안해줘"
```
...
if [ -z "$AGENTS" ] || [ -z "$PROMPT" ] || [ -z "$TOPIC" ]; then
  echo "Error: to=<agentId> topic=<주제> prompt=\"<질문>\" 필수"
  echo "Usage: /delegate to=product-leader topic=q3-strategy prompt=\"시장 분석해줘\""
  exit 1
fi
